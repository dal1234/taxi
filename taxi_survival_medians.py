...
""" Survival analysis on median trip times """

survivor_averages = []
for i in range(0, 2100, 10):
    survivor_averages.append([i, df[df['TIME'] > i]['TIME'].median()])

survivor_averages_df = pd.DataFrame(survivor_averages, columns = ['TIME', 'MEDIAN'])

survival = []

for t in df_test['TIME']:
    for e in range(0, len(survivor_averages_df['TIME'])):
        if t >= 2000:
            survival.append(t)
            break
        elif t >= survivor_averages_df['TIME'][e]:
            continue
        else:
            survival.append(survivor_averages_df['MEDIAN'][e])
            break

df_test['TRAVEL_TIME'] = survival


""" Create submission file """

submission_survival = pd.DataFrame(df_test['TRAVEL_TIME'])
submission_survival.to_csv('C:/Users/dleonard/Documents/Kaggle/Taxi/taxi_survival_medians.csv')